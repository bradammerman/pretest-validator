"""Web login validation - verify web application credentials."""
...
class WebLoginValidator:
    ...
    def validate_login(self, target: dict) -> list[ValidationResult]:
        ...
        username_field = target.get('username_field', 'username')
        password_field = target.get('password_field', 'password')
        ...
        results.append(self._check_page_reachable(name, url, timeout, verify_ssl))
    ...
    def _check_page_reachable(self, name: str, url: str, 
                               timeout: int, verify_ssl: bool) -> ValidationResult:
            ...
            if success_indicator:
                if success_indicator.lower() in response_text:
                    return ValidationResult(
                        name=f"Web Login Auth: {name}",
                        status=ValidationStatus.SUCCESS,
                        message=f"Login successful - found success indicator",
                        details={
                            'status_code': response.status_code,
                            'final_url': response.url,
                        }
                    ...
                    return ValidationResult(
                        name=f"Web Login Auth: {name}",
                        status=ValidationStatus.WARNING,
                        message=f"Login status uncertain - success indicator not found",
                        details={
                            'status_code': response.status_code,
                            'final_url': response.url,
                        }
            ...
            if response.status_code == 200:
                return ValidationResult(
                    ...
                    details={
                        'status_code': response.status_code,
                        'final_url': response.url,
                        'note': 'Configure success_indicator for reliable validation',
                    }
                )
            else:
                return ValidationResult(
